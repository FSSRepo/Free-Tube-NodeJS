...
const { beginVideos, endVideos } = require('../helper');
...
module.exports = {
    ...
    processVideo: async (req,res) => {
        ...
        let videos = await beginVideos();
            ...
            await endVideos(videos);
            ffmpeg.ffprobe(path.resolve("controller/","../uploads/"+id+".mp4"),(err,data) => {
                if(err)console.log(err);
                let random_d = data.format.duration * Math.random();
                let screenshot = "";
            ffmpeg(path.resolve("controller/","../uploads/"+id+".mp4")).on("filenames",(file) => {
                screenshot = file[0];
            }).on("end",() => {
                fs.renameSync('./thumbnails/'+screenshot, './thumbnails/'+id+".png");
                res.status(200).send({});
            }).on('error', function(err) {
                console.log('an error happened: ' + err.message);
              }).takeScreenshots({count:1, timemarks: [random_d.toString()],size: '640x360'},path.resolve("controller/","../thumbnails"));
            });
    },
